...
function generateTOTP(secret: string): string {
  ...
  // In production tests, use a proper library like `otpauth`
...
test.describe('Multi-Factor Authentication (MFA)', () => {
  test('1. Enable MFA with TOTP authenticator', async ({ page }) => {
    ...
    // Should show success
    await expect(
      page.locator('text=/MFA.*enabled/i, text=/two-factor.*active/i')
  ...
  test('5. Disable MFA from settings', async ({ page }) => {
    ...
    // Should show success
    await expect(
      page.locator('text=/MFA.*disabled/i, text=/two-factor.*off/i')
    ).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Session Management & Security', () => {
  ...
  test('2. Session expires after inactivity', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/(login|signin|auth)/);
    await expect(
      page.locator('text=/session.*expired/i, text=/logged.*out/i')
    ).toBeVisible();
  });
  ...
  test('4. View active sessions', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/Chrome/i, text=/Firefox/i, text=/Safari/i')
    ).toBeVisible();
  });
  ...
  test('5. Revoke session from another device', async ({ browser }) => {
    ...
    const revokeButtons = page1.locator('button:has-text("Revoke"), button:has-text("End session")');
...
test.describe('Password Security', () => {
  ...
  test('2. Password requirements enforced', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/uppercase/i, text=/capital.*letter/i')
    ).toBeVisible();
    ...
    await expect(
      page.locator('text=/special.*character/i, text=/symbol/i')
    ).toBeVisible();
  });
  ...
  test('3. Password reset flow', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/check.*email/i, text=/sent.*link/i')
    ).toBeVisible();
...
test.describe('Account Lockout Protection', () => {
  test('1. Lock account after failed login attempts', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/account.*locked/i, text=/too.*many.*attempts/i, text=/temporarily.*disabled/i')
    ).toBeVisible({ timeout: 10000 });
    ...
    await expect(
      page.locator('text=/account.*locked/i')
    ).toBeVisible();
  });
  ...
  test('2. Show remaining attempts before lockout', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/attempts.*remaining/i, text=/3.*more/i')
    ).toBeVisible();
  });
});
