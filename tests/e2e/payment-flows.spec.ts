...
test.describe('Payment Flows - Critical Business Paths', () => {
  ...
  test('1. Complete trial activation with valid payment method', async () => {
    ...
    await expect(
      page.locator('text=/trial.*active/i, text=/welcome/i')
  ...
  test('3. Handle insufficient funds scenario', async () => {
    ...
    await expect(
      page.locator('text=/insufficient.*funds/i, text=/payment.*failed/i')
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('4. View subscription details', async () => {
    ...
    await expect(
      page.locator('text=/next.*billing/i, text=/renews/i')
    ).toBeVisible();
  ...
  test('6. View billing history', async () => {
    ...
    await expect(
      page.locator('text=/billing.*history/i, text=/transactions/i')
    ).toBeVisible();
    
    // Check for table headers
    const headers = ['Date', 'Amount', 'Status', 'Description'];
    for (const header of headers) {
      const headerLocator = page.locator(`th:has-text("${header}"), [data-header="${header.toLowerCase()}"]`);
  ...
  test('7. Initiate subscription cancellation', async () => {
    ...
    await expect(
      page.locator('text=/cancel.*end.*trial/i, text=/access.*until/i')
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('8. Reactivate cancelled subscription', async () => {
    ...
    await expect(
      page.locator('text=/reactivated/i, text=/subscription.*active/i')
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('9. Upgrade to annual plan', async () => {
    ...
    if (await upgradeButton.isVisible()) {
      ...
      await expect(
        page.locator('text=/save.*%/i, text=/year/i')
      ).toBeVisible();
      ...
      await expect(
        page.locator('text=/upgraded/i, text=/annual.*plan/i')
      ).toBeVisible({ timeout: 10000 });
    }
  });
  ...
  test('10. Verify PCI compliance indicators', async () => {
    ...
    await expect(
      page.locator('text=/secure/i, text=/encrypted/i, text=/SSL/i')
    ).toBeVisible();
    
    // Verify Stripe branding (PCI compliance)
    await expect(
      page.locator('text=/Stripe/i, img[alt*="Stripe"]')
    ).toBeVisible();
  ...
  test('13. Verify subscription status after trial end', async () => {
    ...
    await expect(
      page.locator('text=/trial.*ends/i, text=/converts.*on/i')
    ).toBeVisible();
    
    // Verify what happens after trial
    await expect(
      page.locator('text=/charged.*after/i, text=/then.*\$/i')
    ).toBeVisible();
  });
  ...
  test('14. Test add-ons and module upgrades', async () => {
    ...
    if (await addOnsSection.isVisible()) {
      ...
      if (await addButton.isVisible()) {
        ...
        // Should show pricing
        await expect(
          page.locator('text=/\$/i, text=/month/i, text=/price/i')
        ).toBeVisible();
      }
    }
  });
...
test.describe('Payment Error Handling', () => {
  test('Handle Stripe API errors gracefully', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/temporarily.*unavailable/i, text=/try.*later/i')
    ).toBeVisible({ timeout: 10000 });
  });
