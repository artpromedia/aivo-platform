...
const STOP_WORDS = ['image of', 'picture of', 'photo of', 'graphic of', 'icon of', 'screenshot of'];
...
export function validateAltText(
  ...
  if (trimmedAlt.length > 0 && trimmedAlt.length < MIN_ALT_LENGTH) {
    warnings.push(
      `Alt text is very short (${trimmedAlt.length} chars). Consider adding more detail.`
    );
  }
  ...
  for (const stopWord of STOP_WORDS) {
    if (lowerAlt.startsWith(stopWord)) {
      warnings.push(
        `Avoid starting alt text with "${stopWord}" - screen readers already announce images`
      );
      break;
    }
  }
  ...
  if (/click|tap|press/i.test(trimmedAlt)) {
    warnings.push(
      'Avoid interaction instructions in alt text. Use aria-label on clickable elements instead.'
    );
  }
...
export function generateAltTextSuggestion(context: ImageContext): string[] {
  ...
  switch (context.context) {
    ...
    case 'logo':
      suggestions.push('Include the company/brand name: "Aivo logo" or "Aivo Education"');
      break;
 ...
 */
export function formatAltAttribute(altText: string | undefined, isDecorative = false): string {
  if (isDecorative) {
    return '';
  }
  return altText?.trim() || '';
...
export function isLikelyDecorative(context: ImageContext): boolean {
  ...
  const decorativePatterns = [/background/i, /pattern/i, /divider/i, /spacer/i, /decoration/i];
