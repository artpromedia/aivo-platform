...
export function trackConversion(event: string, data?: Record<string, unknown>): void {
  // Google Analytics
  if (typeof window !== 'undefined' && (globalThis as Record<string, unknown>).gtag) {
    (
  ...
  // Vercel Analytics
  if (typeof window !== 'undefined' && (globalThis as Record<string, unknown>).va) {
    (
      globalThis as unknown as Record<string, unknown> & {
        va: (type: string, payload: unknown) => void;
      }
