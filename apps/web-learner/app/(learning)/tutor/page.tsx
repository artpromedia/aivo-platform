...
const SUGGESTED_QUESTIONS = [
  { emoji: '🧮', text: 'Help me with fractions' },
  { emoji: '📖', text: 'Explain the water cycle' },
  { emoji: '✍️', text: 'How do I write a good paragraph?' },
  { emoji: '🔬', text: 'What are the planets in order?' },
];
...
const AI_RESPONSES: Record<string, string> = {
  ...
  water:
    "The water cycle is like nature's recycling system! 💧\n\n1. **Evaporation** ☀️ - The sun heats up water in oceans and lakes, turning it into invisible water vapor that rises into the air.\n\n2. **Condensation** ☁️ - When water vapor gets high in the sky where it's cold, it turns back into tiny water droplets that form clouds.\n\n3. **Precipitation** 🌧️ - When clouds get heavy with water droplets, the water falls back down as rain, snow, or hail.\n\n4. **Collection** 🌊 - Water collects in oceans, lakes, and rivers... and the cycle starts again!\n\nPretty cool, right?",
  paragraph:
...
export default function TutorPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-12rem)] flex-col rounded-2xl border border-slate-200 bg-white shadow-lg">
      {/* Header */}
      <div className="flex items-center gap-4 border-b border-slate-200 bg-gradient-to-r from-purple-600 to-blue-600 p-4 text-white rounded-t-2xl">
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/20 text-2xl">
          🤖
        </div>
        <div>
          <h1 className="text-lg font-bold">AIVO AI Tutor</h1>
          <p className="text-sm text-purple-200">Your personal learning assistant</p>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTED_QUESTIONS.map((q) => (
              <button
                ...
                className="rounded-full bg-slate-100 px-3 py-1.5 text-sm text-slate-700 transition hover:bg-blue-100 hover:text-blue-700"
              >
                {q.emoji} {q.text}
              </button>
            ))}
      ...
      <div className="border-t border-slate-200 p-4">
        ...
        >
          <input
            ...
            className="flex-1 rounded-xl border border-slate-200 px-4 py-3 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
          />
          <button
            type="submit"
