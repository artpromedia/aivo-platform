...
import { gamificationApi, type ActivityStats } from '@/lib/gamification-api';
...
export function RegulationContainer({
  ...
  const handleActivityComplete = useCallback(
    async (activityType: ActivityType, rating?: number) => {
      ...
      try {
        const transaction = await gamificationApi.recordActivityCompletion(
          learnerId,
          activityType,
          {
            rating,
            metadata: {
              source: 'self-regulation',
              gradeBand,
            },
          }
        );
        ...
        if (transaction) {
          // Update stats with actual XP values
          setActivityStats((prev) => ({
            ...prev,
            totalXp: prev.totalXp + transaction.amount,
            level: transaction.newLevel || prev.level,
          }));

          console.log('Activity completed with XP:', {
            activityType,
            rating,
            xpAwarded: transaction.amount,
            leveledUp: transaction.leveledUp,
          });
        }
      } catch (error) {
        console.error('Failed to award XP for activity:', error);
        // Stats were already updated optimistically, so we just log the error
      }
    },
    [learnerId, gradeBand]
  );

  // Load activity stats from gamification service
  useEffect(() => {
    async function loadActivityStats() {
      ...
      try {
        const stats = await gamificationApi.getActivityStats(learnerId);
        setActivityStats(stats);
      } catch (error) {
        console.error('Failed to load gamification stats:', error);
        setStatsError('Failed to load progress data');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats banner - shows loading, error, or gamification stats */}
      {isLoadingStats ? (
        <div className="bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg p-4 text-white">
          <div className="flex items-center justify-center">
            <div className="animate-pulse flex items-center gap-2">
              <div className="w-4 h-4 bg-white/30 rounded-full"></div>
              <span className="text-white/80">Loading your progress...</span>
            </div>
          </div>
        </div>
      ) : statsError ? (
        <div className="bg-gradient-to-r from-slate-400 to-slate-500 rounded-lg p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-white/80 text-sm">Progress unavailable</p>
              <p className="text-sm">{statsError}</p>
            </div>
            <button
              onClick={() => {
                setIsLoadingStats(true);
                gamificationApi.getActivityStats(learnerId).then((stats) => {
                  setActivityStats(stats);
                  setStatsError(null);
                }).catch(() => {
                  setStatsError('Failed to load progress data');
      ...
      ) : activityStats.totalCompleted > 0 || activityStats.totalXp > 0 ? (
        <div className="bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-white/70">
                Level {activityStats.level} - {activityStats.totalXp} XP
              </p>
            </div>
