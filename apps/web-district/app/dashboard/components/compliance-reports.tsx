                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        className={`px-2 py-0.5 text-xs font-medium rounded-full ${getTypeBadge(report.type)}`}
                      >
                        {getTypeLabel(report.type)}
                      </span>
                      <span
                        className={`px-2 py-0.5 text-xs font-medium rounded-full ${getStatusBadge(report.status)}`}
                      >
                        {report.status}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500">
                      Period: {report.period} • {report.schools} schools • Generated{' '}
        ...
        <div className="divide-y divide-gray-100">
          {scheduledReports.map((report) => (
            <div key={report.id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-start gap-4">
                <div
                  className={`p-2 rounded-lg ${report.enabled ? 'bg-green-100' : 'bg-gray-100'}`}
                >
                  <Calendar
                    className={`w-5 h-5 ${report.enabled ? 'text-green-600' : 'text-gray-400'}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
