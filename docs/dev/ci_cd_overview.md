...
- Job `verify` installs deps (Node 22.22.0, pnpm), restores pnpm/turbo caches, and runs `pnpm run verify-all` (lint, tests, type checks/builds). This job must be green before merging to `main`.
