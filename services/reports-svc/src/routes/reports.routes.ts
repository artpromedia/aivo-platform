...
const GenerateReportSchema = z.object({
  ...
  parameters: z
    .object({
      ...
      aggregationLevel: z
        .enum(['student', 'class', 'grade', 'school', 'district'])
        .default('student'),
    })
    .optional()
    .default({}),
...
const reportRoutes: FastifyPluginAsync<ReportRoutesOptions> = async (fastify, options) => {
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      const reportRequest = {
        ...
        dateRange:
          body.dateRange?.startDate && body.dateRange?.endDate
            ? {
  ...
  fastify.post<{
    Body: z.infer<typeof GenerateReportSchema>;
  }>('/generate/stream', async (request, reply) => {
    const user = getUser(request);
    const body = GenerateReportSchema.parse(request.body);
  ...
  }>('/generate/batch', async (request, reply) => {
    const user = getUser(request);
  ...
  fastify.get<{
    Querystring: { tenantId: string };
  }>('/templates', async (_request, reply) => {
    const templates = [
      {
        id: 'student_progress',
