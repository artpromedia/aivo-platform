...
function mapDbRowToInteraction(row: any): AiInteractionSummary {
  ...
  const decisionFactors: AiDecisionFactors = {
    ...
    sessionDurationMinutes:
      safetyMeta.session_duration_minutes ?? safetyMeta.sessionDurationMinutes,
  };
...
function extractSubject(promptSummary?: string): string {
  ...
  if (summary.includes('math') || summary.includes('fraction') || summary.includes('multiply'))
    return 'Math';
  if (summary.includes('read') || summary.includes('story') || summary.includes('comprehension'))
    return 'Reading';
  if (summary.includes('science') || summary.includes('experiment')) return 'Science';
  if (summary.includes('writ') || summary.includes('essay') || summary.includes('paragraph'))
    return 'Writing';
  return 'General';
}
...
function generateExplanation(requestType: string, masteryLevel: number, accuracy: number): string {
  ...
  return (
    explanations[requestType] ??
    `AI assistance provided based on learning data. Student at ${masteryDesc} level.`
  );
}
...
export const registerTeacherTransparencyRoutes: FastifyPluginAsync<
  TeacherTransparencyRoutesOptions
> = async (fastify: FastifyInstance, opts) => {
    ...
    try {
      ...
      if (!hasAccess) {
        reply.code(403).send({
          error: 'Access denied',
          message:
            'You do not have access to this student. The student must be enrolled in one of your classes.',
        });
        return;
      }
    } catch (error) {
  ...
  fastify.post<{
    ...
    reply.code(201).send({
      ...
      status: 'submitted',
      message:
        'Your concern has been submitted for review. Thank you for helping improve our AI systems.',
    });
  });
  ...
  fastify.get<{
    ...
    const summary = {
      ...
      averageInteractionsPerStudent:
        studentsUsingAi > 0 ? totalAiInteractions / studentsUsingAi : 0,
      safetyOverview: {
        safeResponses,
        filteredResponses,
        piiRedactions,
