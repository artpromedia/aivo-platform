...
export class ParentSafetyNotificationService {
   ...
   */
  async notifyParentsOfIncident(incident: SafetyIncidentNotification): Promise<NotificationResult> {
   ...
   */
  private async getParentContacts(learnerId: string, tenantId: string): Promise<ParentContact[]> {
    const result = await this.pool.query(
      `SELECT
         p.id as parent_id,
         p.email,
