/**
 * AIVO Legal Hold Service - Report Service
 *
  ...
  try {
    ...
    await prisma.holdReport.update({
      ...
      data: {
        ...
        filePath: content, // In production, store to S3 and save path
...
export async function getReport(tenantId: string, reportId: string) {
  return prisma.holdReport.findFirst({
...
async function generateHoldStatusReport(
  tenantId: string,
  params: GenerateReportInput['parameters']
): Promise<string> {
  ...
  const holds = await prisma.legalHold.findMany({
    ...
    include: {
      ...
      custodians: {
        where: params.includeReleased ? undefined : { status: { not: 'RELEASED' } },
      },
      dataSources: true,
    },
    orderBy: { createdAt: 'desc' },
  ...
  const rows = holds.map((hold) => {
    const acknowledged = hold.custodians.filter((c) => c.status === 'ACKNOWLEDGED').length;
    const total = hold.custodians.length;
    const complianceRate = total > 0 ? ((acknowledged / total) * 100).toFixed(1) : '100.0';

    return {
      holdNumber: hold.holdNumber,
      name: hold.name,
      matter: hold.matter.matterNumber,
      status: hold.status,
      priority: hold.priority,
      issuedAt: hold.issuedAt?.toISOString() || '',
      totalCustodians: total,
      acknowledgedCustodians: acknowledged,
  ...
  return JSON.stringify({
    reportType: 'HOLD_STATUS',
    generatedAt: new Date().toISOString(),
    totalHolds: rows.length,
    data: rows,
  }, null, 2);
}

async function generateCustodianComplianceReport(
  tenantId: string,
  params: GenerateReportInput['parameters']
): Promise<string> {
...
async function generateAcknowledgmentReport(
  tenantId: string,
  params: GenerateReportInput['parameters']
): Promise<string> {
  ...
  const acknowledgments = await prisma.holdAcknowledgment.findMany({
    ...
    include: {
      hold: {
        select: { holdNumber: true, name: true },
      },
    },
  ...
  const rows = acknowledgments.map((ack) => ({
    ...
    hasQuestions: ack.hasQuestions,
    questions: ack.questions || '',
  }));

  return JSON.stringify({
    reportType: 'ACKNOWLEDGMENT_SUMMARY',
    generatedAt: new Date().toISOString(),
    totalAcknowledgments: rows.length,
    data: rows,
  }, null, 2);
}
...
async function generateDataSourceReport(
  tenantId: string,
  _params: GenerateReportInput['parameters']
): Promise<string> {
  const dataSources = await prisma.dataSource.findMany({
    ...
    include: {
      holds: {
        ...
        include: {
          hold: { select: { holdNumber: true, name: true } },
        },
      },
      custodians: {
        include: {
          custodian: { select: { email: true, displayName: true } },
        },
      },
    },
  });
  ...
  const rows = dataSources.map((ds) => ({
    ...
    canPreserve: ds.canPreserve,
    canExport: ds.canExport,
    canSearch: ds.canSearch,
    activeHolds: ds.holds.length,
    linkedCustodians: ds.custodians.length,
    lastSyncAt: ds.lastSyncAt?.toISOString() || '',
  }));

  return JSON.stringify({
    reportType: 'DATA_SOURCE_INVENTORY',
    generatedAt: new Date().toISOString(),
    totalDataSources: rows.length,
    data: rows,
  }, null, 2);
}

async function generateMatterSummaryReport(
  tenantId: string,
  params: GenerateReportInput['parameters']
): Promise<string> {
  const where: any = { tenantId };
  ...
  const rows = matters.map((matter) => {
    ...
    const compliance = total > 0 ? ((totalAcknowledged / total) * 100).toFixed(1) : '100.0';
    ...
    return {
      ...
      totalHolds: matter.holds.length,
...
async function generateAuditTrailReport(
  ...
  const rows = auditLogs.map((log) => ({
    holdNumber: log.hold.holdNumber,
    holdName: log.hold.name,
    action: log.action,
