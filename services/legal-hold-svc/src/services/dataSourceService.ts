/**
 * AIVO Legal Hold Service - Data Source Service
 *
...
export async function deleteDataSource(tenantId: string, dataSourceId: string) {
  ...
  if (dataSource.holds.length > 0) {
    throw new Error('Cannot delete data source with active holds');
  }
...
export async function addDataSourceToHold(
  ...
  const holdDataSource = await prisma.holdDataSource.upsert({
  ...
  await prisma.holdAuditLog.create({
    data: {
      ...
      description: `Added data source: ${dataSource.name}`,
      performedBy: userId,
    },
  });
...
export async function removeDataSourceFromHold(
  ...
  const holdDataSource = await prisma.holdDataSource.findFirst({
  ...
  await prisma.holdAuditLog.create({
    data: {
      holdId,
      action: 'DATA_SOURCE_RELEASED',
      description: `Released data source: ${holdDataSource.dataSource.name}`,
      performedBy: userId,
    },
  });
}
...
export async function getPreservationSummary(tenantId: string, holdId: string) {
  ...
  for (const hds of hold.dataSources) {
    ...
    if (hds.itemsPreserved) {
      summary.totalItemsPreserved += hds.itemsPreserved;
    }
    if (hds.sizePreserved) {
      summary.totalSizePreserved += hds.sizePreserved;
    }

    summary.sources.push({
      id: hds.id,
      dataSource: {
        id: hds.dataSource.id,
        name: hds.dataSource.name,
        type: hds.dataSource.sourceType,
      },
      status: hds.status,
  ...
  return {
    ...summary,
    totalSizePreserved: summary.totalSizePreserved.toString(),
  };
}
