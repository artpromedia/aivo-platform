/**
 * AIVO Legal Hold Service - Matter Service
 *
...
export async function listMatters(
  ...
  const [matters, totalItems] = await Promise.all([
    prisma.legalMatter.findMany({
      ...
      include: {
        _count: {
          select: { holds: true, custodians: true },
        },
      },
    }),
...
export async function closeMatter(
  ...
  if (matter.holds.length > 0) {
    throw new Error('Cannot close matter with active holds');
  }
...
export async function addCustodianToMatter(
  ...
  // Verify matter and custodian exist in tenant
  ...
  const matterCustodian = await prisma.matterCustodian.upsert({
...
export async function removeCustodianFromMatter(
  ...
  const matterCustodian = await prisma.matterCustodian.findFirst({
  ...
  await logMatterActivity(matterId, 'CUSTODIAN_REMOVED', `Removed custodian: ${matterCustodian.custodian.displayName}`, userId);
}
  ...
  for (const hold of matter.holds) {
    for (const hc of hold.custodians) {
      if (hc.status === 'ACKNOWLEDGED') {
        totalAcknowledged++;
      } else if (['PENDING', 'NOTIFIED'].includes(hc.status)) {
        totalPending++;
      }
    }
  }

  const totalCustodianHolds = totalAcknowledged + totalPending;
  const complianceRate = totalCustodianHolds > 0
    ? (totalAcknowledged / totalCustodianHolds) * 100
    : 100;
...
export async function getMatterActivity(
  ...
  const [activities, totalItems] = await Promise.all([
    prisma.matterActivity.findMany({
      where: { matterId },
