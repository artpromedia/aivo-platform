...
export const config = {
  ...
  // Database
  databaseUrl: process.env.DATABASE_URL ?? 'postgresql://localhost:5432/aivo_experimentation',
  warehouseUrl: process.env.WAREHOUSE_URL ?? 'postgresql://localhost:5432/aivo_warehouse',
  policyDbUrl: process.env.POLICY_DB_URL ?? 'postgresql://localhost:5432/aivo_policies',
