...
export async function registerRoutes(app: FastifyInstance, config: RouteConfig): Promise<void> {
  const { prisma, apiKeyService, webhookDispatcher: _webhookDispatcher } = config;
   ...
   */
  app.get(
    '/public/learners/:learnerId/progress',
    {
      schema: {
        params: z.object({
    ...
    async (request: FastifyRequest<{ Params: { learnerId: string } }>, reply: FastifyReply) => {
      ...
      try {
        ...
        app.log.error(
          { err: message, learnerId: request.params.learnerId },
   ...
   */
  app.get(
    '/public/learners/:learnerId/sessions',
    {
      schema: {
        params: z.object({
          learnerId: z.string().uuid(),
   ...
   */
  app.post(
    '/public/events/external-learning',
    {
      schema: {
    ...
    async (
      request: FastifyRequest<{ Body: z.infer<typeof ExternalLearningEventSchema> }>,
      reply: FastifyReply
    ) => {
      const { apiKeyAuth } = request;
  ...
  app.post(
    ...
    async (
      request: FastifyRequest<{ Body: z.infer<typeof CreateWebhookEndpointSchema> }>,
      reply: FastifyReply
    ) => {
      const tenantId = request.user?.tenantId ?? (request.headers['x-tenant-id'] as string);
    ...
    '/admin/webhooks/:id/deliveries',
    {
      schema: {
        ...
        querystring: z.object({
          ...
          pageSize: z.coerce.number().int().min(1).max(100).default(20),
          status: z
            .enum(['PENDING', 'IN_PROGRESS', 'SUCCESS', 'FAILED', 'PERMANENT_FAILURE'])
            .optional(),
        }),
      },
    },
  ...
  app.delete(
    ...
    async (
      ...
      await apiKeyService.revokeApiKey(request.params.id, userId, request.body?.reason);
  ...
  app.get(
    ...
    async (
      ...
      const avgResponseTime =
        logs.length > 0 ? logs.reduce((sum, l) => sum + l.responseTimeMs, 0) / logs.length : 0;
