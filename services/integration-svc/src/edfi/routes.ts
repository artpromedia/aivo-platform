 ...
 * Ported from edfi-svc into integration-svc.
 */

import type { FastifyInstance } from 'fastify';

import { EdfiClient, type EdfiApiVersion } from './client.js';
import { ExportService, type LearnerDataSource, type EdfiPrismaClient } from './export-service.js';
...
export async function registerEdfiRoutes(
  app: FastifyInstance,
  prisma: EdfiPrismaClient,
  ...
  app.post<{ Params: ConfigParams; Body: { resourceTypes: string[] } }>(
    ...
    async (request, reply) => {
      ...
      const errorsByType: Record<string, { count: number; samples: { recordId: string; field: string; error: string }[] }> = {};
      for (const err of errors) {
        const rt = err.resourceType;
        if (!errorsByType[rt]) {
          errorsByType[rt] = { count: 0, samples: [] };
        }
        errorsByType[rt].count++;
        if (errorsByType[rt].samples.length < 5) {
          errorsByType[rt].samples.push({
...
export default registerEdfiRoutes;
