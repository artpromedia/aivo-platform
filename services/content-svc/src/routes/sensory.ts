...
export async function sensoryRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      const metadata = await sensoryMetadataService.getSensoryMetadataById(request.params.id);
      if (!metadata) {
  ...
  fastify.patch(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      const metadata = await sensoryMetadataService.updateSensoryMetadata(request.params.id, body);
      return metadata;
    }
  );
  ...
  fastify.get(
    ...
    async (
      ...
      const result = await sensoryMetadataService.listSensoryMetadata({
        suitableForPhotosensitive: parseBooleanQuery(query.suitableForPhotosensitive),
        suitableForAudioSensitive: parseBooleanQuery(query.suitableForAudioSensitive),
        suitableForMotionSensitive: parseBooleanQuery(query.suitableForMotionSensitive),
        maxIntensityScore: parseIntQuery(query.maxIntensityScore),
        analyzedBySystem: parseBooleanQuery(query.analyzedBySystem),
        manuallyReviewed: parseBooleanQuery(query.manuallyReviewed),
        page: parseIntQuery(query.page),
        pageSize: parseIntQuery(query.pageSize),
      });
      return result;
    }
  );
   ...
   */
  fastify.post('/sensory-match', async (request: FastifyRequest, reply: FastifyReply) => {
    const body = z
      .object({
    ...
    const matchResult = calculateSensoryMatch(
      body.profile as SensoryProfile,
      contentMetadata,
      body.options
    );
   ...
   */
  fastify.post('/sensory-match/batch', async (request: FastifyRequest, reply: FastifyReply) => {
    const body = z
      .object({
        profile: SensoryProfileSchema,
    ...
    const results = batchCalculateSensoryMatch(
      body.profile as SensoryProfile,
      contentItems,
      body.options
    );
   ...
   */
  fastify.post('/sensory-filter', async (request: FastifyRequest, reply: FastifyReply) => {
    const body = z
      .object({
        profile: SensoryProfileSchema,
        options: z
  ...
  fastify.post('/sensory-incidents', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const incident = await sensoryIncidentService.createSensoryIncident(incidentInput);
  ...
  fastify.get(
    ...
    async (
      ...
      const result = await sensoryIncidentService.listIncidents({
        ...
        systemDetected: parseBooleanQuery(query.systemDetected),
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      const body = ResolveIncidentSchema.parse(request.body);
        ...
        resolutionNotes: body.resolutionNotes,
          actionsTaken: body.actionsTaken?.map(
            (a): IncidentAction => ({
              type: a.type ?? 'unspecified',
              description: a.description ?? '',
              performedAt: new Date(a.performedAt),
              performedByUserId: a.performedByUserId ?? undefined,
            })
          ),
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Params: { contentId: string } }>, reply: FastifyReply) => {
  ...
  fastify.get(
    ...
    async (
      ...
      const threshold = parseIntQuery(request.query.threshold);
...
/**
 * Parse a query string to boolean or undefined.
 */
...
/**
 * Parse a query string to integer or undefined.
 */
...
/**
 * Convert Prisma ContentSensoryMetadata to local type.
 */
...
function prismaToContentMetadata(prismaMetadata: any): ContentSensoryMetadata {
  return {
    ...
    visualComplexity: prismaMetadata.visualComplexity?.toLowerCase() ?? 'moderate',
    ...
    animationIntensity: prismaMetadata.animationIntensity?.toLowerCase() ?? 'none',
    ...
    cognitiveLoad: prismaMetadata.cognitiveLoad?.toLowerCase() ?? 'medium',
