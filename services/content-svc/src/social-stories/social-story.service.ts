...
const ACTIVITY_CATEGORY_MAP: Record<string, SocialStoryCategory> = {
  LESSON: 'STARTING_LESSON' as SocialStoryCategory,
  QUIZ: 'TAKING_QUIZ' as SocialStoryCategory,
  TEST: 'TEST_TAKING' as SocialStoryCategory,
  BREAK: 'SENSORY_BREAK' as SocialStoryCategory,
};

const EMOTIONAL_CATEGORY_MAP: Record<string, SocialStoryCategory> = {
  FRUSTRATED: 'FEELING_FRUSTRATED' as SocialStoryCategory,
  OVERWHELMED: 'FEELING_OVERWHELMED' as SocialStoryCategory,
  ANXIOUS: 'FEELING_ANXIOUS' as SocialStoryCategory,
  ANGRY: 'CALMING_DOWN' as SocialStoryCategory,
  EXCITED: 'CELEBRATING_SUCCESS' as SocialStoryCategory,
};
...
export async function getRecommendations(
  ...
  if (detectedEmotionalState) {
    const category = EMOTIONAL_CATEGORY_MAP[detectedEmotionalState.toUpperCase()];
    if (category) {
      const stories = await prisma.socialStory.findMany({
        where: {
          category,
      ...
      for (const story of stories) {
        ...
        if (!alreadyAdded) {
          recommendations.push({
            ...
            context: { detectedEmotionalState },
          });
        }
      }
    }
...
type PrismaSocialStory = Prisma.SocialStoryGetPayload<Record<string, never>>;

/**
 * Transform a Prisma story to the API response format.
 */
function transformStoryToMetadata(story: PrismaSocialStory): SocialStoryWithMetadata {
  ...
  return {
    ...
    category: story.category as SocialStoryWithMetadata['category'],
    pages,
    readingLevel: story.readingLevel as SocialStoryWithMetadata['readingLevel'],
    estimatedDuration: story.estimatedDuration,
    minAge: story.minAge,
    maxAge: story.maxAge,
    gradeBands: story.gradeBands as SocialStoryWithMetadata['gradeBands'],
    supportsPersonalization: story.supportsPersonalization,
    personalizationTokens: story.personalizationTokens,
    defaultVisualStyle: story.defaultVisualStyle as SocialStoryWithMetadata['defaultVisualStyle'],
...
export class SocialStoryService {
  ...
  getBySlug = getStoryBySlug;
  create = createStory;
  update = updateStory;
  delete = deleteStory;
  approve = approveStory;
  getLearnerPreferences = getLearnerPreferences;
  updateLearnerPreferences = updateLearnerPreferences;
  recordView = recordStoryView;
  getLearnerStats = getLearnerStoryStats;
  getEffectiveness = getStoryEffectiveness;
  createAssignment = createAssignment;
  updateAssignment = updateAssignment;
  deleteAssignment = deleteAssignment;
  getLearnerAssignments = getLearnerAssignments;
  getRecommendations = getRecommendations;
  personalize = personalizeStory;
}
