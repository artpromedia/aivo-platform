...
import type {
  Prisma,
  ContentSensoryMetadata as PrismaContentSensoryMetadata,
...
export async function createSensoryMetadata(
  ...
  const suitableForPhotosensitive = !input.hasFlashing && (input.overallIntensityScore ?? 5) <= 5;
  const suitableForAudioSensitive = !input.hasSuddenSounds && (input.audioLevel ?? 5) <= 5;
  const suitableForMotionSensitive =
    !input.hasAnimation || animationIntensity === 'none' || animationIntensity === 'mild';
 ...
 */
export function analyzeContentForSensory(request: ContentAnalysisRequest): ContentAnalysisResult {
...
export class SensoryMetadataService {
  create = createSensoryMetadata;
  getById = getSensoryMetadataById;
  getByVersionId = getSensoryMetadataByVersionId;
  list = listSensoryMetadata;
  update = updateSensoryMetadata;
  markAsReviewed = markAsReviewed;
  delete = deleteSensoryMetadata;
  analyzeContent = analyzeContentForSensory;
  getVersionsNeedingAnalysis = getVersionsNeedingAnalysis;
  getCoverage = getSensoryAnalysisCoverage;
}
