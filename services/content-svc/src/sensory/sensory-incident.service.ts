...
import type { CreateSensoryIncidentInput, ResolveSensoryIncidentInput } from './sensory.types.js';
...
export async function createSystemDetectedIncident(
  ...
  data: {
    ...
    behavioralSignals: { type: string; value: unknown; timestamp: Date; source: string }[];
    severity?: 'low' | 'medium' | 'high' | 'critical';
  }
): Promise<PrismaSensoryIncident> {
...
export async function listIncidents(
  ...
  if (severity) where.severity = severity.toUpperCase() as 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
  if (status)
...
export async function getIncidentStats(tenantId: string): Promise<IncidentStats> {
  ...
  return {
    total,
    byStatus: Object.fromEntries(byStatus.map((s) => [s.status.toLowerCase(), s._count.status])),
...
export async function getContentWithHighIncidentRate(
  tenantId: string,
  threshold = 5
): Promise<{ contentId: string; incidentCount: number }[]> {
...
export class SensoryIncidentService {
  create = createSensoryIncident;
  createSystemDetected = createSystemDetectedIncident;
  getById = getIncidentById;
  list = listIncidents;
  getRecentForLearner = getRecentIncidentsForLearner;
  getForContent = getIncidentsForContent;
  acknowledge = acknowledgeIncident;
  startInvestigating = startInvestigating;
  resolve = resolveIncident;
  dismiss = dismissIncident;
  addAction = addActionToIncident;
  getStats = getIncidentStats;
  getLearnerStats = getLearnerIncidentStats;
  isContentFlagged = isContentFlagged;
  getHighIncidentContent = getContentWithHighIncidentRate;
}
