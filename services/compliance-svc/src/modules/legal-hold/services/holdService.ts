/**
 * AIVO Compliance Service - Legal Hold Module – Hold Service
 *
...
export async function getHold(tenantId: string, holdId: string) {
  return prisma.legalHold.findFirst({
    ...
    include: {
      matter: {
        select: { id: true, matterNumber: true, name: true, status: true },
      },
      custodians: {
        include: {
          custodian: {
            select: { id: true, email: true, displayName: true, department: true, status: true },
          },
        },
      ...
      _count: {
        select: { custodians: true, notifications: true, acknowledgments: true },
      },
    },
  });
}
...
export async function updateHold(
  ...
  const existing = await prisma.legalHold.findFirst({ where: { id: holdId, tenantId } });
  if (!existing) throw new Error('Hold not found');

  if (['ACTIVE', 'ISSUED'].includes(existing.status)) {
    const restricted = ['scopeDescription', 'dataTypes', 'dateRangeStart', 'dateRangeEnd', 'keywords'];
    const attempted = restricted.filter((f) => f in input);
    if (attempted.length > 0) {
      throw new Error(`Cannot modify ${attempted.join(', ')} on an active hold`);
    }
  }
...
export async function issueHold(
  ...
  // Add custodians
  const custodianOps = input.custodianIds.map((custodianId) =>
    prisma.holdCustodian.upsert({
      where: { holdId_custodianId: { holdId, custodianId } },
      create: { holdId, custodianId, status: 'PENDING', addedBy: userId },
      update: { status: 'PENDING', releasedAt: null, releasedBy: null, releaseReason: null },
    }),
  );

  // Add data sources
  const dsOps = (input.dataSourceIds || []).map((dataSourceId) =>
  ...
  await Promise.all([...custodianOps, ...dsOps]);

  const effectiveDate = input.effectiveAt ? new Date(input.effectiveAt) : new Date();
  const acknowledgmentDue =
    hold.acknowledgmentDue ||
    new Date(effectiveDate.getTime() + config.holds.acknowledgmentDeadlineDays * 24 * 60 * 60 * 1000);

  const updated = await prisma.legalHold.update({
    where: { id: holdId },
    data: { status: 'ISSUED', issuedAt: new Date(), effectiveAt: effectiveDate, acknowledgmentDue },
  });
...
export async function releaseHold(
  ...
  const hold = await prisma.legalHold.findFirst({
    where: { id: holdId, tenantId },
    include: { custodians: true, dataSources: true },
  });
  if (!hold) throw new Error('Hold not found');
  ...
  if (input.releaseType === 'FULL') {
    await Promise.all([
      prisma.holdCustodian.updateMany({
        where: { holdId },
        data: { status: 'RELEASED', releasedAt: new Date(), releasedBy: userId, releaseReason: input.reason },
      }),
...
export async function addCustodianToHold(
  ...
  const hc = await prisma.holdCustodian.upsert({
    where: { holdId_custodianId: { holdId, custodianId } },
    create: { holdId, custodianId, status: 'PENDING', notes: options.notes, addedBy: userId },
    update: { status: 'PENDING', notes: options.notes, releasedAt: null, releasedBy: null, releaseReason: null },
...
export async function removeCustodianFromHold(
  ...
  const hc = await prisma.holdCustodian.findFirst({
    ...
    include: { hold: { select: { tenantId: true } }, custodian: true },
  });
  if (hc?.hold.tenantId !== tenantId) throw new Error('Hold custodian not found');

  await prisma.holdCustodian.update({
    where: { id: hc.id },
    data: { status: 'RELEASED', releasedAt: new Date(), releasedBy: userId, releaseReason: reason },
  });

  await logHoldAudit(holdId, 'CUSTODIAN_RELEASED', `Released custodian: ${hc.custodian.displayName}`, userId);
}
...
export async function getHoldCompliance(
  ...
  const hold = await prisma.legalHold.findFirst({
    where: { id: holdId, tenantId },
    include: { custodians: { where: { status: { not: 'RELEASED' } } } },
  });
  if (!hold) return null;

  const counts = { acknowledged: 0, pending: 0, nonCompliant: 0, escalated: 0 };
  for (const hc of hold.custodians) {
    switch (hc.status) {
      case 'ACKNOWLEDGED': counts.acknowledged++; break;
      case 'PENDING': case 'NOTIFIED': counts.pending++; break;
      case 'NON_COMPLIANT': counts.nonCompliant++; break;
      case 'ESCALATED': counts.escalated++; break;
    }
  }

  const total = hold.custodians.length;
  const rate = total > 0 ? (counts.acknowledged / total) * 100 : 100;
...
export async function getHoldsDashboard(tenantId: string) {
  const [statusCounts, priorityCounts, overdueHolds, recentHolds] = await Promise.all([
    prisma.legalHold.groupBy({ by: ['status'], where: { tenantId }, _count: { status: true } }),
    ...
    prisma.legalHold.count({
      where: { tenantId, status: { in: ['ACTIVE', 'ISSUED'] }, acknowledgmentDue: { lt: new Date() } },
    }),
    prisma.legalHold.findMany({
      where: { tenantId },
      orderBy: { createdAt: 'desc' },
      take: 5,
      include: { matter: { select: { matterNumber: true } }, _count: { select: { custodians: true } } },
    }),
  ]);
