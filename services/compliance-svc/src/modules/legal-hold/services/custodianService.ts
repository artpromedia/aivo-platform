/**
 * AIVO Compliance Service - Legal Hold Module – Custodian Service
 *
 ...
 */
export async function createCustodian(tenantId: string, input: CreateCustodianInput) {
  ...
  return prisma.custodian.findFirst({
    ...
    include: {
      matters: {
        ...
        include: {
          matter: { select: { id: true, matterNumber: true, name: true, status: true } },
        },
      },
      holds: {
        where: { status: { not: 'RELEASED' } },
        include: {
          hold: { select: { id: true, holdNumber: true, name: true, status: true, issuedAt: true } },
        },
      },
      dataSources: {
        include: {
          dataSource: { select: { id: true, name: true, sourceType: true } },
        },
      },
    },
  });
...
export async function updateCustodian(
  ...
  const existing = await prisma.custodian.findFirst({ where: { id: custodianId, tenantId } });
  if (!existing) throw new Error('Custodian not found');

  return prisma.custodian.update({
...
export async function getCustodianHolds(
  ...
  const custodian = await prisma.custodian.findFirst({ where: { id: custodianId, tenantId } });
  if (!custodian) throw new Error('Custodian not found');

  const where: any = { custodianId };
  if (!options.includeReleased) where.status = { not: 'RELEASED' };

  return prisma.holdCustodian.findMany({
    where,
    include: {
      hold: {
        include: {
...
export async function getCustodianCompliance(tenantId: string, custodianId: string) {
  ...
  const holdCustodians = await prisma.holdCustodian.findMany({
    ...
    include: {
      hold: { select: { id: true, holdNumber: true, name: true, acknowledgmentDue: true } },
    },
  });
...
export async function bulkImportCustodians(
  ...
  const result = { imported: 0, skipped: 0, errors: [] as { email: string; error: string }[] };
 ...
 */
export async function searchCustodians(tenantId: string, searchTerm: string, limit = 10) {
...
export async function getCustodiansWithPendingAcknowledgments(
  ...
  const whereClause: any = {
    ...
    hold: { tenantId, status: { in: ['ISSUED', 'ACTIVE'] } },
  };
  if (overdueOnly) whereClause.hold.acknowledgmentDue = { lt: new Date() };

  const [holdCustodians, totalItems] = await Promise.all([
    prisma.holdCustodian.findMany({
      where: whereClause,
      include: {
        custodian: { select: { id: true, email: true, displayName: true, department: true } },
        hold: { select: { id: true, holdNumber: true, name: true, acknowledgmentDue: true } },
      },
      orderBy: { addedAt: 'asc' },
      skip,
