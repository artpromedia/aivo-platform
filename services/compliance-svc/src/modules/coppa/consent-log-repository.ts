/**
 * COPPA/Consent Module — Consent Log Repository
 * Immutable audit trail for all consent actions (grants and revocations).
...
function mapConsentLog(row: ConsentLogRow): ConsentLogRecord {
  return {
    id: row.id,
    ...
    status: row.status,
    source: row.source as ConsentLogRecord['source'],
...
): Promise<{ logs: ConsentLogRecord[]; total: number }> {
