/**
 * COPPA/Consent Module — Repository (raw pg queries)
...
function mapConsentLink(row: ConsentLinkRow): ParentalConsentLinkRecord {
  return {
    id: row.id,
    tenant_id: row.tenant_id,
    learner_id: row.learner_id,
...
function mapVerificationMethod(row: VerificationMethodRow): VerificationMethodRecord {
  return {
    id: row.id,
    consent_id: row.consent_id,
    tenant_id: row.tenant_id,
...
export async function applyTransition(
  ...
  try {
    ...
    const nextConsent: ConsentRecord = { ...consent, ...transition.updates } as ConsentRecord;
...
export async function checkConsentStatus(
  ...
  learnerId: string,
  consentType: ConsentTypeValue,
): Promise<ConsentStatusCacheRecord | null> {
  const { rows } = await pool.query<ConsentStatusCacheRecord>(
...
export async function checkMultipleConsents(
  ...
  tenantId: string,
  learnerId: string,
  consentTypes: ConsentTypeValue[],
): Promise<Map<ConsentTypeValue, ConsentStatusCacheRecord>> {
  ...
  const result = new Map<ConsentTypeValue, ConsentStatusCacheRecord>();
...
): Promise<ParentalConsentLinkRecord> {
...
export async function getConsentLinkByToken(
  pool: Pool,
  tokenHash: string,
): Promise<ParentalConsentLinkRecord | null> {
  const { rows } = await pool.query<ConsentLinkRow>(
...
export async function getVerificationForConsent(
  pool: Pool,
  consentId: string,
): Promise<VerificationMethodRecord | null> {
  const { rows } = await pool.query<VerificationMethodRow>(
...
export async function getTenantCoppaSettings(
  pool: Pool,
  tenantId: string,
): Promise<TenantCoppaSettingsRecord> {
...
export async function upsertTenantCoppaSettings(
  pool: Pool,
  settings: Partial<TenantCoppaSettingsRecord> & { tenant_id: string },
): Promise<TenantCoppaSettingsRecord> {
  const { rows } = await pool.query<CoppaSettingsRow>(
...
export async function findExpiredConsents(pool: Pool, limit = 100): Promise<ConsentRecord[]> {
  const { rows } = await pool.query<ConsentRow>(
