/**
 * DSR Exporter – Build GDPR/COPPA-compliant export bundles
 *
 * Includes all personal data, AI interactions, and consent history.
 * Export bundle v2.0 adds consent records (GDPR Art 15) and
...
export async function buildExportBundle(
  ...
  const exportEventLimit = config.exportEventLimit ?? 500;
  ...
  const eventsResult = await pool.query(
    `SELECT id, event_type, created_at, metadata - 'raw_payload' - 'actor_email' - 'actor_name' AS metadata
     ...
     LIMIT $3`,
    [tenantId, learnerId, exportEventLimit],
  );
  ...
  // Consent records (GDPR Article 15 / COPPA)
  ...
  // Parental consent verification history (COPPA)
