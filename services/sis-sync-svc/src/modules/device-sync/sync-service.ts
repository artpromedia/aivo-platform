 ...
 * Refactored to accept PrismaClient via constructor (DI pattern).
 */
...
import {
  ...
  type PushResult,
  type PullResult,
  type DeviceSyncConflict as SyncConflict,
  type ServerChange,
...
export class DeviceSyncService {
  ...
  async pushChanges(
    ...
    const rejectedOperations: { id: string; reason: string }[] = [];
    const conflicts: SyncConflict[] = [];

    // Process operations in batches
    const batches = this.batchOperations(operations, deviceSyncConfig.sync.batchSize);
  ...
  private async processOperation(
    ...
    if (serverEntity && serverEntity.version >= operation.clientVersion) {
      const conflict = await this.createConflict(ctx, operation, serverEntity, tx);

      // Attempt auto-resolution if enabled
      if (deviceSyncConfig.features.autoConflictResolution) {
  ...
  private async applyOperation(
    ...
    switch (operation.operation) {
      ...
      default:
        throw new Error(`Unknown operation type: ${String(operation.operation)}`);
    }
  }
  ...
  async pullChanges(
    ...
    for (const entityType of types) {
      const entityChanges = await this.getEntityChanges(ctx, entityType, since, limit);
    ...
    return {
      ...
      hasMore,
      serverTimestamp,
      nextCursor: hasMore ? changes[limit - 1]?.timestamp : undefined,
    };
  }
  ...
  async getDeltaChanges(
    ...
    const serverEntity = await this.getServerEntity(ctx, entityType, entityId);
  ...
  async getPendingConflicts(ctx: AuthContext): Promise<SyncConflict[]> {
    const conflicts = await this.prisma.deviceSyncConflict.findMany({
      where: {
      ...
      take: deviceSyncConfig.sync.maxConflicts,
    });
  ...
  async resolveConflict(
    ...
    const conflict = await this.prisma.deviceSyncConflict.findUnique({
      where: { id: conflictId },
    });
    ...
    await this.prisma.$transaction(async (txn: any) => {
      ...
      await txn.deviceSyncConflict.update({
        where: { id: conflictId },
        data: {
  ...
  private async createConflict(
    ...
    const conflict = await tx.deviceSyncConflict.create({
      data: {
        tenantId: ctx.tenantId,
        userId: ctx.userId,
        entityType: operation.entityType,
  ...
  private async applyResolution(
    ctx: AuthContext,
    resolved: { data: Record<string, unknown>; conflictId: string },
    tx: any
  ): Promise<void> {
    const conflict = await tx.deviceSyncConflict.findUnique({
    ...
    await tx.deviceSyncConflict.update({
      where: { id: resolved.conflictId },
      data: {
        status: ConflictStatus.RESOLVED,
        resolvedAt: new Date(),
   ...
   */
  private isConflictingField(entityType: EntityType, field: string): boolean {
  ...
  private async recordSyncHistory(
    ...
    await this.prisma.deviceSyncHistory.create({
      data: {
        tenantId: ctx.tenantId,
        userId: ctx.userId,
        deviceId: ctx.deviceId,
