...
const PRIORITY_KEYWORDS = {
  ...
  P2: ['should', 'nice to have', 'improvement', 'when needed', 'when available'],
  P3: ['cleanup', 'refactor', 'eventually', 'later', 'someday', 'optional', 'tech debt'],
};
...
function assignPriority(content, category) {
  const lowerContent = content.toLowerCase();
...
function isStale(filePath, content) {
  ...
  const stalePatterns = [/2023/, /2024/, /phase 1/i, /sprint 0/i, /later/i, /someday/i, /temporary/i];

  return stalePatterns.some((pattern) => pattern.test(content));
}

function extractTodoType(line) {
...
function generateReport(items) {
  ...
  // Initialize counters
  const categories = ['security', 'performance', 'bug', 'feature', 'cleanup', 'unknown'];
...
function generateMarkdownReport(report) {
  ...
  if (p0Items.length === 0) {
    ...
    p0Items.forEach((item) => {
      ...
      const shortContent =
        item.content.length > 60 ? item.content.slice(0, 57) + '...' : item.content;
      lines.push(
  ...
  if (p1Items.length === 0) {
    ...
    p1Items.slice(0, 20).forEach((item) => {
      ...
      const shortContent =
        item.content.length > 50 ? item.content.slice(0, 47) + '...' : item.content;
      lines.push(
        `| [${shortFile}](${item.file}#L${item.line}) | ${item.line} | ${item.category} | ${shortContent} | ${item.owner} |`
      );
    });
  ...
  if (staleItems.length === 0) {
    ...
    staleItems.slice(0, 15).forEach((item) => {
      ...
      lines.push(
        `| [${shortFile}](${item.file}#L${item.line}) | ${item.line} | ${shortContent} | Potentially outdated |`
      );
    });
