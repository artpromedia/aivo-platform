...
export function createTracer(config) {
    const { serviceName, serviceVersion = '1.0.0', environment = 'development', enabled = true, } = config;
    ...
    return {
        ...
        setAivoContext(ctx) {
            ...
            if (!span)
                return;
            if (ctx.tenantId)
                span.setAttribute(AIVO_ATTRIBUTES.TENANT_ID, ctx.tenantId);
            if (ctx.userId)
                span.setAttribute(AIVO_ATTRIBUTES.USER_ID, ctx.userId);
            if (ctx.learnerId)
                span.setAttribute(AIVO_ATTRIBUTES.LEARNER_ID, ctx.learnerId);
            if (ctx.requestId)
                span.setAttribute(AIVO_ATTRIBUTES.REQUEST_ID, ctx.requestId);
            if (ctx.sessionId)
                span.setAttribute(AIVO_ATTRIBUTES.SESSION_ID, ctx.sessionId);
            if (ctx.agentType)
                span.setAttribute(AIVO_ATTRIBUTES.AI_AGENT_TYPE, ctx.agentType);
            if (ctx.provider)
                span.setAttribute(AIVO_ATTRIBUTES.AI_PROVIDER, ctx.provider);
            if (ctx.model)
                span.setAttribute(AIVO_ATTRIBUTES.AI_MODEL, ctx.model);
        },
        async shutdown() {
...
export function httpSpanAttributes(req) {
    const attrs = {};
    if (req.method)
        attrs[AIVO_ATTRIBUTES.HTTP_METHOD] = req.method;
    if (req.url)
        attrs[AIVO_ATTRIBUTES.HTTP_URL] = req.url;
    if (req.route)
        attrs[AIVO_ATTRIBUTES.HTTP_ROUTE] = req.route;
    if (req.host)
        attrs[AIVO_ATTRIBUTES.HTTP_HOST] = req.host;
    if (req.userAgent)
        attrs[AIVO_ATTRIBUTES.HTTP_USER_AGENT] = req.userAgent;
    return attrs;
}
/**
