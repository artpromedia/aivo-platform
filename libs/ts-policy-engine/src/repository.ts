...
function mapRow(row: PolicyDocumentRow): PolicyDocument {
  return {
    id: row.id,
...
export class PolicyRepository {
   ...
   */
  async getActiveGlobalPolicy(): Promise<PolicyDocument | null> {
    const { rows } = await this.pool.query<PolicyDocumentRow>(
   ...
   */
  async getActiveTenantPolicy(tenantId: string): Promise<PolicyDocument | null> {
    const { rows } = await this.pool.query<PolicyDocumentRow>(
      `SELECT * FROM policy_documents 
   ...
   */
  async listActiveTenantPolicies(
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      this.pool.query<PolicyDocumentRow>(
        `SELECT * FROM policy_documents 
   ...
   */
  async create(input: CreatePolicyDocumentInput): Promise<PolicyDocument> {
    ...
    try {
      ...
      const versionResult = await client.query<{ max_version: number }>(
        `SELECT COALESCE(MAX(version), 0) as max_version 
         ...
         WHERE scope_type = $1 AND (tenant_id = $2 OR (tenant_id IS NULL AND $2 IS NULL))`,
        [input.scopeType, input.tenantId ?? null]
      );
      const nextVersion = (versionResult.rows[0]?.max_version ?? 0) + 1;

      // Insert new document
      const { rows } = await client.query<PolicyDocumentRow>(
      ...
      // Log the creation
      await client.query(
        `INSERT INTO policy_audit_logs (policy_document_id, action, performed_by_user_id, new_values)
         VALUES ($1, 'CREATED', $2, $3)`,
        [rows[0]!.id, input.createdByUserId ?? null, JSON.stringify(input.policyJson)]
      );
   ...
   */
  async activate(id: string, activatedByUserId?: string): Promise<PolicyDocument> {
    const client = await this.pool.connect();
    try {
      await client.query('BEGIN');

      // Get the document to activate
      const { rows: docRows } = await client.query<PolicyDocumentRow>(
        `SELECT * FROM policy_documents WHERE id = $1`,
        [id]
      );
      if (docRows.length === 0) {
      ...
      await client.query(
        `UPDATE policy_documents 
         ...
         WHERE scope_type = $1 
           ...
           AND is_active = true
           AND id != $3`,
        [doc.scope_type, doc.tenant_id, id]
      );
      ...
      // Log the activation
      await client.query(
        `INSERT INTO policy_audit_logs (policy_document_id, action, performed_by_user_id)
         VALUES ($1, 'ACTIVATED', $2)`,
        [id, activatedByUserId ?? null]
      );
   ...
   */
  async deactivate(id: string, deactivatedByUserId?: string): Promise<PolicyDocument> {
    const client = await this.pool.connect();
    try {
      await client.query('BEGIN');
  ...
  async update(
    ...
    try {
      ...
      const { rows: currentRows } = await client.query<PolicyDocumentRow>(
        `SELECT * FROM policy_documents WHERE id = $1`,
        [id]
      );
      ...
      const { rows } = await client.query<PolicyDocumentRow>(
        `UPDATE policy_documents SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
        params
      );
  ...
  async delete(id: string): Promise<void> {
    const { rowCount } = await this.pool.query(
      `DELETE FROM policy_documents WHERE id = $1 AND is_active = false`,
      [id]
    );
    if (rowCount === 0) {
