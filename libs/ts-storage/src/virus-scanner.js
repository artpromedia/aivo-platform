...
export class ClamAVScanner {
    ...
    async scanBuffer(content, filename) {
    ...
    parseResult(response, scanDurationMs, _filename) {
...
export class VirusTotalScanner {
    ...
    async waitForResult(analysisId, maxAttempts = 30) {
...
export class MockVirusScanner {
    infectedPatterns;
    simulateDelay;
    constructor(infectedPatterns = [/eicar/i, /test.?virus/i], simulateDelay = 100) {
    ...
    async scanStreamToBuffer(stream, filename) {
        const chunks = [];
        for await (const chunk of stream) {
            chunks.push(chunk);
        }
 ...
 */
export function createVirusScanner(config) {
    ...
    switch (config.provider) {
        ...
        case 'mock':
            return new MockVirusScanner();
        default:
            throw new Error(`Unknown virus scanner provider: ${config.provider}`);
    }
}
//# sourceMappingURL=virus-scanner.js.map