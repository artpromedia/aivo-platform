...
export type LearningObjectSubject =
  | 'ELA'
  | 'MATH'
  | 'SCIENCE'
  | 'SEL'
...
export type LearningObjectGradeBand =
...
export type LearningObjectVersionState =
...
export type LearningObjectContentType =
  | 'reading_passage'
  | 'reading_passage_with_questions'
  | 'math_problem'
  | 'math_problem_set'
  | 'sel_check_in'
  | 'sel_scenario'
...
// ══════════════════════════════════════════════════════════════════════════════
// CONTENT JSON TYPES (discriminated union by "type")
// ══════════════════════════════════════════════════════════════════════════════
...
export interface SelOption {
  ...
  emoji?: string;
  imageUrl?: string;
  value: number; // For tracking/analytics
}
...
export interface InteractiveGameContent extends ContentJsonBase {
  ...
  body: {
    gameType: string; // e.g., "matching", "sorting", "drag_drop", "word_builder"
    configUrl?: string; // URL to game config/assets
    config?: Record<string, unknown>; // Inline game configuration
    estimatedPlayTimeMinutes: number;
  };
}

// ── Speech Exercise Content ──────────────────────────────────────────────────
...
export type LearningObjectContentJson =
  | ReadingPassageContent
  | ReadingPassageWithQuestionsContent
  | MathProblemContent
  | MathProblemSetContent
  | SelCheckInContent
  | SelScenarioContent
  | VideoLessonContent
  | InteractiveGameContent
  | SpeechExerciseContent
...
export interface AccessibilityMetadata {
  ...
  supports?: {
    dyslexia?: boolean;      // Dyslexia-friendly formatting available
    adhd?: boolean;          // ADHD accommodations (chunking, etc.)
    visualImpairment?: boolean; // Screen reader optimized
    hearingImpairment?: boolean; // Captions/transcripts available
    motorImpairment?: boolean;  // Keyboard navigation support
  };
...
export interface LearningObjectMetadata {
  ...
  /** License information */
  license?: {
    type: string; // e.g., "CC-BY-4.0", "proprietary"
    attribution?: string;
  };
...
export interface CreateLearningObjectVersionInput {
  ...
  standardsJson?: StandardsAlignment;
  metadataJson?: LearningObjectMetadata;
  skillAlignments?: { skillId: string; weight?: number }[];
}
...
export interface ListLearningObjectsQuery {
  ...
  skillId?: string;
  tags?: string[];
  includeGlobal?: boolean; // Include tenant_id = null content
  isActive?: boolean;
  page?: number;
  pageSize?: number;
}
