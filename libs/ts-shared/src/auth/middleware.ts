 ...
 */
export function createFastifyAuthHook(config: AuthMiddlewareConfig) {
  const { jwtService, redis, skipPaths = [], requireTenant = true } = config;
  ...
  return async function authHook(
    ...
    try {
      ...
      (request as AuthenticatedFastifyRequest).token = token;
    } catch (error) {
      if (error instanceof Error) {
        if (error.message.includes('expired') || error.name === 'JWTExpired') {
...
export function requireTenantMatch(tenantIdParam: string = 'tenantId') {
  return function tenantMiddleware(
    ...
    const requestedTenantId =
      req.params[tenantIdParam] || (req.body as Record<string, unknown>)?.tenantId;
 ...
 */
export function createServiceAuthMiddleware(jwtService: JWTService, serviceName: string) {
  return async function serviceAuthMiddleware(
    ...
    try {
      ...
      (req as Request & { service: { name: string; target: string } }).service = {
