      ...
      <Card className={cn('w-full', className)}>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            {tool.icon && <img src={tool.icon} alt={`${tool.name} tool icon`} className="h-6 w-6" />}
            {tool.name}
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      {tools.map((tool) => (
        ...
        >
          <CardContent className="flex items-start gap-4 p-4">
            {tool.icon ? (
              <img src={tool.icon} alt={`${tool.name} learning tool icon`} className="h-12 w-12 rounded object-contain" />
            ) : (
              <div className="flex h-12 w-12 items-center justify-center rounded bg-muted" aria-label={`${tool.name} external tool`}>
