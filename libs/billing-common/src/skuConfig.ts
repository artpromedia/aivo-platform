 ...
 */
export function buildSkuCatalog(): SkuCatalog {
  validateStripeConfig();

  const skus: Record<ParentSku, SkuConfig> = {
    BASE: {
      sku: 'BASE',
...
let _catalog: SkuCatalog | null = null;
 ...
 */
export function resetSkuCatalog(): void {
  _catalog = null;
}
 ...
 */
export function getSkuConfig(sku: ParentSku): SkuConfig {
 ...
 */
export function getStripePriceId(sku: ParentSku, period: 'monthly' | 'yearly'): string {
 ...
 * Get all add-on SKUs
 */
export function getAddonSkus(): ParentSku[] {
 ...
 * Get the base SKU
 */
export function getBaseSku(): ParentSku {
  return 'BASE';
}
...
): { totalCents: number; breakdown: { sku: ParentSku; cents: number }[] } {
 ...
 */
export function validateSkuSelection(skus: ParentSku[]): { valid: boolean; error?: string } {
  if (skus.length === 0) {
 ...
 */
export function getIncludedFeatures(skus: ParentSku[]): string[] {
